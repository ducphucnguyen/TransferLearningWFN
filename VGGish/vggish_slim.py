...
def define_vggish_slim(features_tensor=None, training=False):
  ...
  with slim.arg_scope([slim.conv2d, slim.fully_connected],
    ...
    net1 = slim.conv2d(net, 64, scope='conv1')
    net1 = slim.max_pool2d(net1, scope='pool1')
    net2 = slim.conv2d(net1, 128, scope='conv2')
    net2 = slim.max_pool2d(net2, scope='pool2')
    net3 = slim.repeat(net2, 2, slim.conv2d, 256, scope='conv3')
    net3 = slim.max_pool2d(net3, scope='pool3')
    net4 = slim.repeat(net3, 2, slim.conv2d, 512, scope='conv4')
    net4 = slim.max_pool2d(net4, scope='pool4')

    # Flatten before entering fully-connected layers
    net5 = slim.flatten(net4)
    net6 = slim.repeat(net5, 2, slim.fully_connected, 4096, scope='fc1')
    # The embedding layer.
    net7 = slim.fully_connected(net6, params.EMBEDDING_SIZE, scope='fc2',
                               activation_fn=None)
    #return tf.identity(net1, name='embedding')
    #return tf.identity(net2, name='embedding')
    #return tf.identity(net3, name='embedding')
    #return tf.identity(net4, name='embedding')
    #return tf.identity(net7, name='embedding')
